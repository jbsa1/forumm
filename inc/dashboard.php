...
?>
<title><?php echo $data2["nama_user"];?> | Beranda - REKAT</title>
<?php
}
else{
    ?>
<title>Beranda -  REKAT</title>
...
<div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        if (@$_SESSION["user"]) {
                        ?>
                        <b><?php echo $data2["nama_user"];?> (<?php echo $data2["user_user"];?>)</b>! <a href="./?p=posting&profil=<?php echo $data2["user_user"];?>"><li class="fa fa-pencil"></li> Buat Postingan</a> Di REKAT
                        <?php
                        }
                        else{
                            ?>
...
<?php
                ...
                $lala = mysqli_query($db, "SELECT*FROM komentar WHERE id_post='$datasqlpost[id_post]'");
                    ...
                    ?>
                        <button class="btn btn-primary" onclick="window.location='./inc/like.php?id=<?php echo $datasqlpost["id_post"];?>&u=<?php echo $datasqlpost["penulis_post"];?>';"><i class="fa fa-thumbs-o-up"></i> Kasih Jempol</button>
                    <?php
                        }
                    }else{
                    ?>
